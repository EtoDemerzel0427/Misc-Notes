...
def scene_create():
    """
    初始温度一律先-3，目标温度一律25，风速2；达到12度的时候调整风速为3；达到18度调整目标温度为24（初始20）。达到24保持12秒调整目标温度为28（初始24）.
    :return:
    """
    scene = dict()
    random.seed()
    ...
    for room_name in b4.room_names[1:]:
        ...
        # 12度前稳定升温
        for t in range(it + temp_step, 12 + temp_step, temp_step):
            tc = tc + (4 - w)
            scene_add_expect(scene, tc + 1, 's', f'r={room_name} tc={tc} t={t}')

        # 在到达12度后一个tick改变风速（之前风速为2，一个tick温度还没改变）
        tc, w = tc + 1, 1
        scene_add_action(scene, tc, room_name, f'w={w} tc={tc}')

        # 从这一时刻开始重新变化温度，直到18度
        for t in range(12 +temp_step, 18 + temp_step, temp_step):
            tc = tc + (4 - w)
            scene_add_expect(scene, tc + 1, 's', f'r={room_name} tc={tc} t={t}')

        tc = tc + 1
        # 到达18度的同一个tick改变目标温度
        it, tt = 20, 24 # 由于prj没说重新改变温度的时候it是不是当前温度，这里先假定不是
        scene_add_action(scene, tc, room_name, f'it={it} tt={tt} tc={tc}')
        scene_add_expect(scene, tc + 1, 's', f'r={room_name} tc={tc} t={it}')
        ...
        # 三段：-3到18（中途改变风速）；20到24，保持12s;24到28，保持10s。
        bill = 21 * 1 + 4 * (1 + 12) + 4 * (1 + 10)
